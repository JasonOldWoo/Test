...
{ 
    ...
    { 
	printf("%s **** family(%d)/rtm_table(%d) error!\n", __FUNCTION__, rtMsg->rtm_family, rtMsg->rtm_table);
	printf("%s **** should be: %d, %d\n", __FUNCTION__, AF_INET, RT_TABLE_MAIN);
        return -1; 
    } else {
    }
    ...
    { 
        ...
        case RTA_OIF: 
            ...
            printf("ifName: %s, index: %d\n", rtInfo->ifName, *(int*) RTA_DATA(rtAttr));
            break; 
        case RTA_GATEWAY: 
...
{ 
    ...
    { 
	printf("inet_pton error! lpszIpAddr: %s\n", lpszIpAddr);
        goto END; 
    } 
...
int main()
{
	SetLocalNetInfo("eth0", "192.168.68.191", "255.255.255.0");
	return 0;
}
